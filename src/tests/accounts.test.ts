...
import { createAppAccount } from "@/lib/services/accounts";
...
describe("createAppAccount", () => {
  ...
  it("creates an unverified caretaker with starter services", async () => {
    ...
    const session = await createAppAccount(repo, accountCreateSchema.parse(caretakerInput), NOW);
  ...
  it("starts provider numbers at prov_01 in an empty store", async () => {
    const session = await createAppAccount(new MemoryRepository(createSeedData(NOW)), accountCreateSchema.parse(caretakerInput), NOW);
    expect(session.providerId).toBe("prov_01");
  });

  it("gives each caretaker the next provider number", async () => {
    const repo = newRepo();
    const first = await createAppAccount(repo, accountCreateSchema.parse(caretakerInput), NOW);
    const second = await createAppAccount(repo, accountCreateSchema.parse({ ...caretakerInput, email: "other@example.com" }), NOW);
  ...
  it("rejects an email that already has an account", async () => {
    ...
    await expect(createAppAccount(newRepo(), input, NOW)).rejects.toThrow(/already exists/);
  });
