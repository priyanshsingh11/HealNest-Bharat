...
import { localizeCareService, localizeCategory } from "@/lib/i18n/messages/domain";
...
export default async function DiscoverPage({ searchParams }: PageProps) {
  ...
  const t = discoverMessages[locale].page;

  const repo = getRepository();
  const [categories, config, languages] = await Promise.all([repo.listCategories(), repo.getPlatformConfig(), listLanguages(repo)]);
  const activeCategories = bookableCategories(categories).map((c) => localizeCategory(c, locale));
  const selected = activeCategories.find((c) => c.id === params.category);
  const foundService = findCareService(params.service);
  const careService = foundService && localizeCareService(foundService, locale);
  /** With a category chosen, only the services that category actually provides are offered. */
  const offeredServices = (selected ? CARE_SERVICES.filter((s) => s.providedBy.includes(selected.id)) : CARE_SERVICES).map((s) =>
    localizeCareService(s, locale),
  );
    ...
    <>
      ...
      <div className="mx-auto max-w-7xl px-4 py-6 sm:px-6 sm:py-8">
        <div className="grid gap-6 lg:grid-cols-[1fr_minmax(0,28rem)] lg:items-start">
          ...
          <div className="lg:pt-7">
            <h1 className="text-2xl font-extrabold tracking-tight text-ink sm:text-3xl">
              {careService ? careService.name : selected ? selected.name : t.careProviders}
              {location && <span className="text-ink-muted">{t.near(location.label)}</span>}
            </h1>
        ...
        <div className="mt-6 space-y-4">
          <div>
            <p className="mb-2 text-xs font-bold tracking-wider text-ink-muted uppercase">{t.whoCanHelp}</p>
            <AutoScrollRow
          ...
          <div className={cn(offeredServices.length === 0 && "hidden")}>
            <p className="mb-2 text-xs font-bold tracking-wider text-ink-muted uppercase">{t.whatYouNeed}</p>
            <AutoScrollRow
        ...
        {!location || !discovery ? (
          <div className="mt-10 rounded-2xl border border-line bg-white px-6 py-14 text-center shadow-sm">
            ...
            <h2 className="text-lg font-bold text-ink">{t.setLocationTitle}</h2>
            <p className="mt-1 text-sm text-ink-muted">{t.setLocationHint}</p>
          </div>
        ) : (
          <div className="mt-6">
            <section aria-labelledby="results-heading" className="min-w-0">
              ...
              <div className="mt-4 mb-4 flex flex-wrap items-center justify-between gap-3 border-t border-line pt-4">
                <h2 id="results-heading" className="font-bold text-ink" aria-live="polite">
                  {t.available(discovery.results.length)}
                </h2>
                {discovery.results.length > 0 && (
                  <MapPanel
                    ...
                    markers={[
                      { id: "you", latitude: location.latitude, longitude: location.longitude, label: t.youMarker(location.label), kind: "user" },
              ...
              {discovery.results.length === 0 ? (
                <div className="rounded-2xl border border-line bg-white p-8 text-center">
                  ...
                  <h3 className="mt-3 text-lg font-bold">{t.noMatchTitle}</h3>
                  <p className="mx-auto mt-1 max-w-md text-sm text-ink-muted">
                    {discovery.totalInCategory > 0
                      ? t.hiddenByFilters(discovery.totalInCategory)
                      : t.noneInArea}
                  </p>
